...
{
    public static Sequence Jump(Transform target, Vector3 endValue, float duration, float height, int numJumps = 1, Ease ease = Ease.Linear)
    {
        ...
        var result = Sequence.Create().Group(jumpsSequence);
        
        if (!Mathf.Approximately(target.position.x, endValue.x))
        {
            result.Group(Tween.PositionX(target, endValue.x, duration, ease));
        }
        
        if (!Mathf.Approximately(target.position.z, endValue.z))
        {
            result.Group(Tween.PositionZ(target, endValue.z, duration, ease));
        }
